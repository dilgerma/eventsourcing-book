...
class CartwithproductsReadModelTest : BaseIntegrationTest() {
    ...
    fun `CartwithproductsReadModelTest`() {
        ...
        awaitUntilAssserted {
            var readModel =
                queryGateway.query(CartsWithProductsReadModelQuery(productId), CartsWithProductsReadModel::class.java)
            assertThat(readModel.get().data).hasSize(1)
            assertThat(readModel.get().data).first()
                .matches { it.aggregateId == aggregateId && it.productId == productId }
        }
    }
