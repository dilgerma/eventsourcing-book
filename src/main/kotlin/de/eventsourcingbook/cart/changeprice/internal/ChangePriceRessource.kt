...
data class ChangePricePayload(	var newPrice:Double,
	var oldPrice:Double,
	var productId:UUID)
...
class ChangePriceRessource(private var commandGateway: CommandGateway) {
    ...
    fun processDebugCommand(@RequestParam newPrice:Double,
	@RequestParam oldPrice:Double,
	@RequestParam productId:UUID):CompletableFuture<CommandResult> {
        return commandGateway.send(ChangePriceCommand(productId, newPrice,
	oldPrice))
    }
    ...
    fun processCommand(
         ...
         return commandGateway.send(ChangePriceCommand(			newPrice=payload.newPrice,
